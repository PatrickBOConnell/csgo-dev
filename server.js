		...
		try {
			socket.get('room', function(err, room){
				...
				do {
					...
					var timestamp = (new Date).getTime();
					shasum.update(room.room + timestamp + i);
					var locid = shasum.digest('hex');
					console.log(locid);
					i++;
				}
				while(rooms[room.room].frames[locid] !== undefined);
				rooms[room.room].frames[locid] = new Frame(locid);
				io.sockets.in(room.room).emit('frame made', {id: locid});
			});
		} catch(e) {
			console.error(e);
		}
