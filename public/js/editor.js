...
$(document).ready(function(){
	...
	$('body').on('dblclick', 'div.frame', function(e){
		console.log('dbclicked frame');
	});
	
	$('body').on('click', 'div.frame', function(e) {
		removeAllFrameBorders();
		$(this).css('border', '2px solid black');
		selectedFrame = this.id;
		console.log('selected frame id: ' + selectedFrame);
	});
	...
	socket.on('frame made', function(data) {
		$('#toolbar').append("<td><div class='frame' id='" + data.id + "'><input type='checkbox' class='framevis'></div></td>");
		framehash[data.id] = new Frame(data.id);
	});
	...
	removeAllFrameBorders = function() {
